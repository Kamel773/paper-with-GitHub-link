...
def is_github_link(link):
	return re.match('^.*zenodo\..*', link)  # Simple regex that checks for URLs matching (an optional prefix ending with '.', plus) 'github.' followed by any suffix

in_path = sys.argv[1] if len(sys.argv) > 1 else "/Volumes/Kamel/Other_Semantic_Scholar"
out_path = sys.argv[2] if len(sys.argv) > 2 else "/Users/kamel/Desktop/CMU/Progress_Dataset/Zenodo/Others_Duplicate/Extracted_links_zenodo.txt"
